...
store_fd_in:
movl %eax, ST_FD_IN(%esi) # Save the given file descriptor.
...
store_fd_out:
movl %eax, ST_FD_OUT(%esi) # Save the file descriptor.

loop_begin:
movl $SYS_READ, %eax # Read system call.
movl ST_FD_IN(%esi), %ebx # Input file descriptor goes to %ebx.
...
movl ST_FD_OUT(%esi), %ebx # Output file descriptor goes to %ebx.
